    ...
    <nav class="dark:bg-green-900">
        <div class="max-w-screen-xl flex flex-wrap mx-auto p-4">
            ...
            <div class="flex items-center lg:order-2 space-x-1 lg:space-x-0 rtl:space-x-reverse ml-auto">
                ...
                // Asegurarse de que existen ambos nombres
                $firstName = isset($nameParts[0]) ? $nameParts[0] : '';
                $lastName = isset($nameParts[1]) ? $nameParts[1] : '';

                // Iniciales: comprobar si existe el apellido
                $initial = strtoupper(substr($firstName, 0, 1));
                $initial2 = $lastName ? strtoupper(substr($lastName, 0, 1)) : '';

                // Limitar el largo de los nombres
                $name = ucfirst(strtolower(substr($firstName, 0, 12)));
                $lastName = $lastName ? ucfirst(strtolower(substr($lastName, 0, 12))) : '';

                $nameLength = strlen($firstName);
                @endphp

                <button type="button" data-dropdown-toggle="language-dropdown-menu" class="inline-flex items-center font-medium justify-center px-4 py-2 text-sm text-gray-900 dark:text-white rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800">
                    {{ $name }} {{ $lastName }}
                </button>
                @endguest
